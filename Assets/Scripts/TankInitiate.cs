...
public class TankInitiate : MonoBehaviour {
    ...
    void Start()
    {
        ...
        for (int i = 1; i < (playersPerTank); i++)
        {
            GameObject myPlayer = Instantiate(subPlayer, transform.position, transform.rotation) as GameObject;
            myPlayer.transform.SetParent(transform);
            var myPlayerScript = myPlayer.GetComponent<ArmFamilySpawner>();
            myPlayerScript.playerNumberInTank = i;
            myPlayerScript.setClass(myClass);
